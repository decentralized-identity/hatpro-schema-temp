...
// do NOT redeclare emitEnums; the base script defines `let emitEnums = false;`
if (has("--emitEnums") || argv.includes("--emitEnums=true")) { emitEnums = true; }
const emitGenEnumSchemas = has("--emitGenEnumSchemas"); // default: false
const isGenEnumName = (name) => /_genEnum$/.test(name);
/**
...
// v1.2: explicit targetPath resolver (literal path under packages/)
function parseEnumTargetPath(pLike, baseId, repoRoot) {
  if (!pLike) return null;
  if (!pLike.startsWith("/")) pLike = "/" + String(pLike).replace(/^\/+/, "");
  const parts = pLike.replace(/^\//, "").split("/").filter(Boolean);
  if (parts.length < 2) return null;
  const seg  = parts[0];
  const name = parts[parts.length - 1];
  const subs = parts.slice(1, -1);
  const $id     = baseId + [seg, ...subs, name].join("/") + ".json"; // caller may override if enumId provided
  const fileAbs = path.join(repoRoot, "packages", seg, ...subs, `${name}.json`);
  ...
  for (let raw of lines) {
    ...
    if (currentField) {
      ...
      if (kv) {
        ...
        // SPECIAL: nested enumDefine block
        if (k === 'enumDefine' && v === '') {
          const obj = {};
          while (lines.length) {
            const peek = lines[0];
            if (!/^\s{2,}\S/.test(peek)) break; // stop on non-indented
            const raw2 = lines.shift();
            const line2 = raw2.replace(/\t/g, '  ');
            const kv2 = line2.match(/^\s*([A-Za-z][A-Za-z0-9_-]*)\s*:\s*(.*)\s*$/);
            if (!kv2) continue;
            let [, k2, v2] = kv2; v2 = String(v2).trim();
            if (/^\[.*\]$/.test(v2)) {
          ...
          hints.fields[currentField][k] = obj;
          continue;
        }
...
function schemaFrom(hints, className, ctx) {
  ...
  for (const [prop, spec] of Object.entries(hints.fields || {})) {
    ...
    if (spec.enumDefine && typeof spec.enumDefine === "object") {
      ...
      // Identity for $id
      const enumId = ed.enumId || ed.path || ed.id || ed.ref;
      // Output location
      const targetPath = ed.targetPath || null;
      let ep;
      if (targetPath) {
        ep = parseEnumTargetPath(String(targetPath), baseId, repoRoot);
      } else {
        ep = parseEnumPathLike(String(enumId), ctx);
      }
      if (ep) {
        // $id prefers enumId if supplied
        const $id = enumId ? (baseId + String(enumId).replace(/^\//, '') + ".json") : ep.$id;
        out.$ref = $id;
        ...
        if (emitEnums && ed.generate === true) {
          ...
          // choose fileAbs based on targetPath if present
          const fileAbs = targetPath ? parseEnumTargetPath(String(targetPath), baseId, repoRoot).fileAbs : ep.fileAbs;
          queueEnumEmit({ fileAbs, node });
        }
      }
    }
    }
