...
// NEW: explicit targetPath resolver (literal path under packages/)
function parseEnumTargetPath(pLike) {
  if (!pLike) return null;
      ...
      if (spec.$ref) {
        ...
        if (["string","number","integer","boolean","object","array","null"].includes(t)) {
          ...
          if (t === "array" && spec.items) {
            ...
            if (spec.items.enumDefine && typeof spec.items.enumDefine === "object") {
              const ed = spec.items.enumDefine;
              const target = ed.targetPath || ed.path || ed.id || ed.ref;
              const ep = target && String(target).startsWith("/")
                ? parseEnumTargetPath(String(target))
                : parseEnumPathLike(String(target), ctx);
              if (ep) {
                it.$ref = ep.$id;
                if (emitEnums && ed.generate === true) {
                  const node = {
